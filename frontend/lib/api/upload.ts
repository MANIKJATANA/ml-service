...
/**
 * Upload one event photo (decisions/0034): mint a per-event signed target, PUT the bytes
 * straight to Supabase, then register the object as a `media` row. Returns the created
 * media. Images only in v1 (`media_type: "image"`); registering enqueues nothing —
 * processing is the separate event-level "Process" action.
 */
export async function uploadEventMedia(
  ...
  return registerMedia(eventId, object_path, "image");
}
