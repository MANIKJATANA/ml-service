...
| Script | What it does |
|---|---|
| `up.ps1` | Brings the stack up in Docker. Backing services (Postgres + Redis) start **detached and keep running**; the app services (frontend + backend + ml-service) run **attached in the foreground**. Press **Ctrl+C** to stop only the app services — Postgres/Redis stay up so you keep DB/queue state. Fails fast if the Docker daemon isn't running. `-Detached` runs the apps in the background too; `-NoBuild` skips rebuilding app images. |
